	...
	public String newUser(User user) {
		try {
			String str_query = String.format(
					"insert into %s (fullname, username, password, email, phone) values(?,?,?,?,?)", Views.TBL_USER);
			String hashPassword = SecurityUtility.encryptBcrypt(user.getPassword());
			int rowaccept = db.update(str_query,
					new Object[] { user.getFullname(), user.getUsername(), hashPassword, user.getEmail(), user.getPhone() });
			if (rowaccept == 1) {
				return "success";
			}
			return "failed";
